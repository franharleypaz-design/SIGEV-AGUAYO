...
// 🕵️‍♂️ DETECTOR MULTI-TENANT DINÁMICO PARA EL LAYOUT CON RESPALDO DE SESIÓN GLOBAL (PASAPORTE)
...
const queryTenantLocal = urlParamsLocal.get('t') || urlParamsLocal.get('tenant');
// Lee primero la URL, si no hay URL lee la memoria, si no hay memoria usa Paz en Localhost
const tenantActual = queryTenantLocal ? queryTenantLocal.toLowerCase().trim() : (sessionStorage.getItem('SIGEV_ACTIVE_TENANT') || ((subdominioDetectado === 'localhost' || subdominioDetectado === '127') ? "paz" : subdominioDetectado));

// ==============================================================================
// 0. APLICACIÓN DE COLORES DINÁMICOS AL INSTANTE (SOLO MODO CLARO CORPORATIVO)
// ==============================================================================
...
// FORZAMOS LA PALETA "CLARA" PERMANENTEMENTE PARA TODA LA PLATAFORMA
...
<aside class="sidebar" style="background-color: var(--sidebar-dinamico); transition: background-color 0.4s ease;">
            <button id="btn-close-sidebar-mobile" style="position: absolute; top: 135px; right: 16px; background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255, 255, 255, 0.25); color: #ffffff; cursor: pointer; display: none; align-items: center; justify-content: center; padding: 6px; border-radius: 50%; width: 32px; height: 32px; box-shadow: 0 2px 8px rgba(0,0,0,0.2); z-index: 99999 !important;" title="Cerrar menú">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.8" width="16" height="16"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
...
<div class="sidebar-menu-overlay" id="sidebar-overlay-tap" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(15, 23, 42, 0.4); backdrop-filter: blur(2px); z-index: 9990; display: none;"></div>`;
...
<header class="top-bar" style="display: flex; align-items: center; justify-content: space-between; transition: background-color 0.4s ease, border-color 0.4s ease;">
    ...
    <div class="top-bar-actions" style="display: flex; align-items: center; flex: 1; justify-content: flex-end;">
        <div id="nav-btn-notificaciones" style="position: relative; cursor: pointer; margin-right: 24px; display: flex; align-items: center; justify-content: center; color: #64748b;" title="Ver notificaciones">
            ...
            <span class="notif-badge" style="position: absolute; top: -3px; right: -3px; background-color: #ef4444; color: white; font-size: 9px; font-weight: 800; height: 16px; width: 16px; border-radius: 50%; display: none; align-items: center; justify-content: center; border: 2px solid #fff; box-shadow: 0 2px 4px rgba(239, 68, 68, 0.3);">0</span>
                ...
                <div id="notif-dropdown" style="display: none; position: absolute; top: 35px; right: -10px; width: 340px; background: #fff; border: 1px solid var(--border-color); border-radius: 8px; box-shadow: 0 8px 24px rgba(0,0,0,0.12); z-index: 1000; cursor: default; text-align: left;">
                                <div style="padding: 12px 16px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
                                    <h4 style="margin: 0; font-size: 14px; color: var(--text-dark);">Requerimientos Pendientes</h4>
                                    <div style="display: flex; align-items: center; gap: 10px;">
                                        <span style="font-size: 11.5px; color: var(--primary-blue); cursor: pointer; font-weight: 700;" id="notif-mark-read">Marcar como leídas ✔️</span>
                                    </div>
                                </div>                <div id="notif-items-body" style="max-height: 280px; overflow-y: auto;">
                    </div>
...
export async function inyectarEstructuraGlobal() {
    ...
    try {
        ...
        if (configSnap.exists()) {
            ...
            document.documentElement.style.setProperty('--header-generico', colorHeaderFinal);
            ...
            // SOLO MODO CLARO CORPORATIVO ACTIVADO
...
// --- ACTUALIZACIÓN DINÁMICA DE LA CAMPANA GLOBAL (MULTI-TENANT EN VIVO) ---
async function inicializarNotificaciones() {
    ...
    // Apertura inmediata sin bloqueos asíncronos para respuesta veloz al clic
    campanaBtn.onclick = (e) => {
        e.stopPropagation();
    ...
    // 🟢 DESACOPLE DE ÍNDICES COMPUESTOS: Consultamos solo por tenantId y filtramos los estados en JS local
    try {
        const qNotif = query(
            collection(db, "solicitudes"),
            where("tenantId", "==", tenantActual)
        );
        ...
        snap.forEach(d => {
            const data = d.data();
            // Filtramos localmente para evitar la exigencia de Indexación Compuesta de Firebase
            if (["Abierta", "En revisión"].includes(data.estado)) {
                solicitudes.push({ id: d.id, ...data });
            }
        });

        // Ordenamiento local rápido y corte de lote a las 5 más recientes
        solicitudes.sort((a, b) => (b.fechaCreacion?.seconds || 0) - (a.fechaCreacion?.seconds || 0));
        const limitadas = solicitudes.slice(0, 5);
        ...
        if (notifBody) {
            if (limitadas.length > 0) {
                let htmlItems = "";
                limitadas.forEach(s => {
                    const fecha = s.fechaCreacion ? new Date(s.fechaCreacion.seconds * 1000).toLocaleDateString('es-CL') : "Reciente";
                    htmlItems += `
                        <div style="padding: 12px 16px; border-bottom: 1px solid #f1f5f9; display: flex; gap: 12px; align-items: flex-start; transition: background 0.2s; cursor: pointer;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='#fff'" onclick="window.location.href='solicitudes.html'">
                            <div style="background: rgba(239, 68, 68, 0.1); color: #ef4444; padding: 6px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>
                            </div>
                            <div style="flex: 1; min-width: 0;">
                                <p style="margin: 0; font-size: 12.5px; color: var(--text-dark); line-height: 1.4; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
                                    <strong>${s.motivo || 'Solicitud'}:</strong> ${s.nombreVecino || 'Vecino'} ingresó un nuevo requerimiento.
                                </p>
                                <span style="font-size: 10px; color: var(--text-light);">${fecha}</span>
                            </div>
                        </div>`;
                });
                notifBody.innerHTML = htmlItems;
            } else {
                notifBody.innerHTML = `<div style="padding: 32px 16px; text-align: center; color: #64748b; font-size: 13px;">✨ ¡Al día! No tienes requerimientos pendientes.</div>`;
            }
        }
    } catch (err) {
        console.error("Error cargando notificaciones:", err);
        if (notifBody) notifBody.innerHTML = `<div style="padding: 20px; text-align: center; color: #94a3b8; font-size: 12px;">Alertas operativas listas en bandeja.</div>`;
    }
}
...
function inicializarMenuMobile() {
    ...
    const btnCerrar = document.getElementById("btn-close-sidebar-mobile");
    ...
    if (btnHamburguesa && workspace) {
        btnHamburguesa.onclick = (e) => {
            ...
            if (capaOscura) capaOscura.style.display = workspace.classList.contains("sidebar-open") ? "block" : "none";
        };
    }

    const cerrarMenu = () => {
        if (workspace) workspace.classList.remove("sidebar-open");
        if (capaOscura) capaOscura.style.display = "none";
    };

    if (btnCerrar) btnCerrar.onclick = cerrarMenu;
    if (capaOscura) capaOscura.onclick = cerrarMenu;
}
